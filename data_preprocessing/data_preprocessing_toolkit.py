...
class DataPreprocessingToolkit(object):
    ...
    def filter_out_company_clients(df):
        ...
        """
        df_copy=df.copy()
        df_copy = df_copy.loc[df_copy['is_company'] == 0]
        return df_copy
    ...
    def filter_out_long_stays(df):
        ...
        """
        df_copy=df.copy()
        df_copy = df_copy.loc[df_copy['length_of_stay'] <= 21]
        return df_copy
    ...
    def add_book_to_arrival(df):
        ...
        """
        df_copy=df.copy()
        #df['book_to_arrival'] = ((df.date_from - df.booking_date) / pd.Timedelta(1, 'd'))
        df_copy["book_to_arrival"] = (df_copy["date_from"] - df_copy["booking_date"]).dt.days
        #df['book_to_arrival'].apply(lambda x: float((df.date_from - df.booking_date).dt.days))
        return df_copy
    ...
    def add_weekend_stay(df):
        ...
        df['weekend_stay'] = (((date_from>=4)&(date_from!=6))|(date_to>=5)|((date_to<date_from)&(date_from!=6))|(difference>=6))
    ...
    def add_night_price(df):
        ...
        """
        df_copy=df.copy()
        df_copy['night_price']=((df_copy['accommodation_price']/df_copy['n_rooms'])/df_copy['length_of_stay']).round(decimals=2)
        return df_copy
    ...
    def aggregate_group_reservations(self, df):
        ...
        aggregate_datas = [group_reservations[['group_id']+self.sum_columns].groupby('group_id').sum(),
                           group_reservations[['group_id']+self.mean_columns].groupby('group_id').mean(),
                           group_reservations[['group_id']+self.mode_columns].groupby('group_id').agg(lambda x: x.value_counts().index[0]),
                           group_reservations[['group_id']+self.first_columns].groupby('group_id').first()]
        group_reservations=aggregate_datas[0]
        group_reservations = group_reservations.merge(aggregate_datas[1], on='group_id')
        group_reservations=group_reservations.merge(aggregate_datas[2],on='group_id')
        group_reservations = group_reservations.merge(aggregate_datas[3], on='group_id')
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        df['room_segment']=df['termnight_price'].apply(lambda x: self.map_value_to_bucket(x,self.room_segment_buckets))
